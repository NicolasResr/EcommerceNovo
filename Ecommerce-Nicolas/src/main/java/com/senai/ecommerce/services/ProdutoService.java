...
public class ProdutoService {
	...
	public List<DTOproduto> buscarTodos() {
		List<Produto> list = repo.findAll();
		return list.stream().map(x -> new DTOproduto(x)).toList();
	}

	public Page<DTOproduto> buscarPagina(Pageable pagina) {
		Page<Produto> result = repo.findAll(pagina);
		return result.map(x -> new DTOproduto(x));
	}
